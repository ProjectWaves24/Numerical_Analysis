...
int_simpson  =  ((p-n)/6.0)*(fun(n) +  4.0*fun(m) +  fun(p));
  ...
  for i = 1:N
	    func_exact(i) = 1.0 - fun(x_exact(i));
end
...
for i = 1:N_calc
	  frac1 = ((x_calc(i)- m)*(x_calc(i)- p))/((n - m)*(n-p));
         ...
         fun_calc(i) = 1.0 - fun(n)*frac1 + fun(m)*frac2 + fun(p)*frac3;
...
error2 = abs(Q-int_trap)
  ...
  legend('exact ','Area (integral)','approximation ');
           ...
           title(['Trapetsmetoden:',num2str(int_trap), 'Adaptive integral:',num2str(Q),'error:',num2str(error2)]);
