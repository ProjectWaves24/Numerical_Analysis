...
h=(ts-t0)/N
t=np.linspace(t0,ts,int(N)+1)
y1=np.linspace(t0,ts,int(N)+1)
y2=np.linspace(t0,ts,int(N)+1)
...
plt.title('Framåt Eulers metod vs. ode45 (h=0.025)')

plt.text(0.5,-20,'dy_1/dt= y_2  +2t, dy_2/dt= y_1 - 2t y_1 + y_2 -t +1');
plt.text(0.5,-23,'y_1(0)=0,y_2(0)=-1');